...
from gui.empleados.empleado_info_window import InfoEmpleado


class EmpleadosFrame(ctk.CTkFrame):
    """Frame principal para la gestión de empleados."""
    ...
    def _crear_filtros(self):
        ...
        for i, campo in enumerate(self.campos_empleado[1:]):  # Excluye el ID
            entry = ctk.CTkEntry(
                entries_frame,
                font=("Lato Light", 16),
                width=150,
    ...
    def _crear_tabla(self):
        ...
        cols = len(self.campos_empleado) - 1
    ...
    def info_empleado(self, info):
        ...
        empleado = self.table.get_row(info["row"])
        row_empleado=info["row"]

        info_empleado = InfoEmpleado(empleado=empleado, table=self.table, row_empleado=row_empleado)